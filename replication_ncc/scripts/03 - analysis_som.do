...
twoway (dropline Comp1 var_nr , horizontal mlabel(Comp1) mlabformat(%12.2f) mlabposition(top)) , ylabel(1(1)6, valuelabel) xlabel(0(0.2)0.6) xtitle(loadings) ytitle("") ysize(3)
...
graph export "$working_ANALYSIS/results/figures/fig_s2.tif", replace
restore
...
preserve
pca negotiate1 negotiate2 negotiate4 negotiate5 negotiate6 negotiate7, comp(1)
matrix list e(L)
matrix define loadings = e(L)
...
twoway (dropline Comp1 var_nr , horizontal mlabel(Comp1) mlabformat(%12.2f) mlabposition(top)) , ylabel(1(1)6, valuelabel) xlabel(0(0.2)0.6) xtitle(loadings) ytitle("") ysize(3)
gr_edit yaxis1.edit_tick 1 6 "Assuming multilateral agreements as the only solution to the climate problem.", tickset(major)
...
graph export "$working_ANALYSIS/results/figures/fig_s4.tif", replace
restore
...
graph save "$output\home_oecd_cdp", replace
...
graph combine "$output\home_ldc_cdp" "$output\home_oecd_cdp" "$output\now_fut", scale(*1.1)  imargin(tiny)
graph export "$working_ANALYSIS/results/figures/fig_s6.tif", replace
...
graph export "$working_ANALYSIS/results/figures/fig_s7.tif", replace
...
* Combine A&B
graph combine "$output/env_pca_items" "$output\dist_env.gph", rows(2)  ysize(5)
