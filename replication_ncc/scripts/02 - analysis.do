...
*---------------------------------------
*   I) FIGURE 1: Temperature rise deemed acceptable, carbon price suggested, and negotiation strategy applied
*------------------------------
// Figure Settings
* Text size
...
legend(order(1 "Observation" 2 "Mean" 3 "Median") size($legend pt) rows(3) pos(6) ring(1) region(fcolor(%40)))
...
// Fig S2a: Distribution of answers towards technological optimism items
...
// Fig. 2b. Coefficient plots temperature rise acceptable and price deemed necessary 
* Set globals
...
margins, at(tech_ms_pca = (`tech_ms_min' (1) `tech_ms_max' )) saving("$output\probit_margins_temp2.gph", replace)
...
margins, dydx(tech_ms_pca)
margins, at(tech_ms_pca = (`tech_ms_min' (1) `tech_ms_max' )) saving("$output\probit_margins_price2.gph", replace)

* Coefplot
combomarginsplot "$output\probit_margins_temp2.gph" "$output\probit_margins_price2.gph", labels("Temperature rise {&ge} 2°C acceptable" "Global carbon price < 60 USD suggested")  xtitle("Technological optimism mindset (PCA)", size($xytitle3)) title("{bf:b }", size($head3) pos(11) ring(15))  xla(-4(2)2, labsize($xylab3)) yla(, labsize($xylab3))  ytitle("Effects on probability of" "accepting/suggesting", size($xytitle3)) recast(line) recastci(rarea) file1opts(msymbol(S) msize(6pt)) file2opts(msize(6pt)) fileci1opts(recast(rarea) lw(none) fcolor(%20)) fileci2opts(recast(rarea) lw(none) fcolor(%20)) legend(rows(2) pos(11) ring(0) size($legend3) region(fcolor(%20))) graphregion(margin(30 30 0 0)) plotregion(margin(0 0 0 0)) fysize(40) scale(*0.5)
...
// Combine graph a & b
graph combine "$working_ANALYSIS/results/intermediate/tech_opt_answers"  "$working_ANALYSIS/results/intermediate/margins_temp_price.gph" , rows(2) ysize(5)
